...
my_print(self):` that prints the square with the character `#` to standard output (if `size` == 0 -> prints an empty line).
...
Python class `Square` that defines a square. Builds on [101-square.py](./101-square.py) with:
